...
dataset = 'wino'
TRAIN_DATA_PATH = f'./data/classifier_train/{dataset}_cot_label.json'
OUTPUT_DIR = f"./result/verifier/{dataset}"
tokenizer = AutoTokenizer.from_pretrained(MODEL_PATH)
model = DebertaV2ForSequenceClassification.from_pretrained(MODEL_PATH, device_map=device_map)
cnt = 0
...
def collate_fn_for_critic(batch):
    ...
    for data in batch:
        label = data['match']
        cot = data['cot']
        encoding = tokenizer(cot, truncation=True, padding="max_length",
...
validation_cycle = int(((len(train_dataloader.dataset)-1)// train_batch_size + 1) // 2)
if is_main_process:
    mlflow.set_experiment("train_verifier")
    mlflow.start_run()
    mlflow.set_tags({
